...
function renderWelcome() {
  ...
  return `<section class="scene hero-stage v3-hero"><div class="hero-layout"><div>
    ...
    <p class="lede">A live simulation of how people recognize uncertainty, select a search surface, reformulate questions, test credibility, and move toward action across Google, AI, local, video, communities, reviews, social search, and owned experiences.</p>
...
function renderCognition() {
  ...
  return `<section class="scene cognition-scene">${sceneHeader('ACT I · COGNITIVE SEARCH REACTOR','Search begins before a keyword is <em>fully formed.</em>','This model translates information-seeking research into practical SEO decisions. It is a behavioral framework, not a claim that every person follows one fixed sequence.')}
    ...
    <section class="panel decision-simulator"><div class="flex-between"><div><p class="eyebrow">DECISION-PATH SIMULATION</p><h2>Change the conditions. Watch the expected path change.</h2></div><div class="scenario-tabs">${COGNITIVE_SCENARIOS.map((item)=>`<button type="button" data-action="cognitive-scenario" data-scenario="${item.id}" class="chip-button${item.id===scenario.id?' is-active':''}">${esc(item.label)}</button>`).join('')}</div></div><div class="divider"></div>
      ...
      <form id="cognition-profile-form" class="simulator-controls"><label><span>Perceived risk <output id="risk-output">${state.cognitiveDraft.risk}</output>/5</span><input name="risk" data-cognitive-slider="risk" type="range" min="1" max="5" value="${state.cognitiveDraft.risk}"></label><label><span>Urgency <output id="urgency-output">${state.cognitiveDraft.urgency}</output>/5</span><input name="urgency" data-cognitive-slider="urgency" type="range" min="1" max="5" value="${state.cognitiveDraft.urgency}"></label><label><span>Category familiarity <output id="familiarity-output">${state.cognitiveDraft.familiarity}</output>/5</span><input name="familiarity" data-cognitive-slider="familiarity" type="range" min="1" max="5" value="${state.cognitiveDraft.familiarity}"></label><label><span>Most valuable next surface</span><select name="next" data-cognitive-next>${optionMarkup(PLATFORMS)}</select></label><button class="primary-button" type="submit">Save my decision profile</button></form>
      <aside class="room-model"><p class="eyebrow">ROOM MODEL</p><div class="room-meter"><label>Risk <b>${avgRisk?avgRisk.toFixed(1):'—'}</b></label><span><i style="width:${avgRisk/5*100}%"></i></span></div><div class="room-meter"><label>Urgency <b>${avgUrgency?avgUrgency.toFixed(1):'—'}</b></label><span><i style="width:${avgUrgency/5*100}%"></i></span></div><div class="room-meter"><label>Familiarity <b>${avgFamiliarity?avgFamiliarity.toFixed(1):'—'}</b></label><span><i style="width:${avgFamiliarity/5*100}%"></i></span></div><p>${profiles.length} participant profile${profiles.length===1?'':'s'} saved for this scenario.</p><div class="reference-interpretation"><strong>Reference interpretation</strong><p>${esc(scenario.interpretation)}</p></div></aside></div></section>
...
function renderJourney() {
  ...
  return `<section class="scene journey-simulator">${sceneHeader('ACT II · DECISION JOURNEY SIMULATOR','The journey is a sequence of <em>functions</em>, not a rigid funnel.','Select a decision stage to inspect the information gap, mental task, evidence requirement, likely surface role, and remaining uncertainty.')}${scenarioTabs}
    ...
    <div class="panel-grid" style="margin-top:16px"><section class="panel span-7 journey-detail-panel"><p class="eyebrow">${esc(detail.label)} · ${esc(detail.function)}</p><h2>${esc(detail.question)}</h2><div class="journey-detail-grid"><div><small>MENTAL TASK</small><p>${esc(detail.mentalTask)}</p></div><div><small>EVIDENCE REQUIRED</small><p>${esc(detail.evidence)}</p></div><div><small>LIKELY SURFACE ROLE</small><p>${esc(detail.surfaceRole)}</p></div><div><small>BREEZY EXAMPLE</small><p>${esc(detail.exampleBreezy)}</p></div><div><small>K&P EXAMPLE</small><p>${esc(detail.exampleKp)}</p></div><div><small>STRATEGIC FAILURE MODE</small><p>${detail.id==='trigger'?'Optimizing only the query while ignoring what created the need.':detail.id==='validate'?'Repeating brand claims when the user is seeking independent corroboration.':'Treating the stage as a channel rather than a decision function.'}</p></div></div></section>
      <aside class="panel span-5 prediction-panel"><p class="eyebrow">NEXT-SURFACE PREDICTION</p><h3>Where is this person most likely to seek the next increment of evidence?</h3><p>${esc(scenario.interpretation)}</p><div class="prediction-grid">${scenario.path.slice(1).map((id)=>{const platform=byPlatform(id);const count=counts.get(id)||0;return `<button type="button" data-action="journey-predict" data-surface="${id}" class="prediction-option${myPrediction&&payloadOf(myPrediction).surface===id?' is-selected':''}"><i style="background:${platform?.color}">${esc(platform?.icon)}</i><strong>${esc(platform?.short||id)}</strong><span><b style="width:${count/max*100}%"></b></span><output>${count}</output></button>`}).join('')}</div><div class="system-note"><strong>Prediction discipline</strong><br>Defend the surface based on the information gap and expected evidence value, not personal preference.</div></aside></div>
    <section class="panel transition-lab"><div class="flex-between"><div><p class="eyebrow">TRANSITION LAB</p><h3>Add one defensible transition insight</h3></div><span class="tag direction">Shared strategic evidence</span></div><form id="journey-insight-form" class="transition-form"><select name="from">${optionMarkup(JOURNEY_STAGES,'id','label')}</select><span>→</span><select name="to">${optionMarkup(JOURNEY_STAGES,'id','label')}</select><input name="insight" maxlength="280" required placeholder="What new information or contradiction moves the person forward, backward, or sideways?"><button class="primary-button" type="submit">Add insight</button></form><div class="transition-wall">${transitions.map((item)=>{const p=payloadOf(item);return `<article><span>${esc(byJourneyStage(p.from)?.label||p.from)} → ${esc(byJourneyStage(p.to)?.label||p.to)}</span><p>${esc(p.insight)}</p><small>${esc(p.ownerName)}</small></article>`}).join('')||'<div class="empty-state"><div><strong>No transition evidence has been added</strong><span>Identify the new information, contradiction, or proof requirement that changes the next move.</span></div></div>'}</div></section>
...
function renderWhiteboard() {
  ...
  return `<section class="scene">${sceneHeader('ACT III · SHARED EVIDENCE MAP','Turn evidence into a <em>visible decision system.</em>','Drag cards across the journey. Everyone sees contributions, movement, priority votes, and cursors live, without refreshing.', `<div class="client-switch" role="group"><button type="button" data-action="board-client" data-client="breezy" class="${state.selectedClient === 'breezy'?'is-active':''}">Breezy</button><button type="button" data-action="board-client" data-client="kp" class="${state.selectedClient === 'kp'?'is-active':''}">KP</button><button type="button" data-action="board-client" data-client="both" class="${state.selectedClient === 'both'?'is-active':''}">Both</button></div>`)}
    ...
    <div class="whiteboard-shell" id="whiteboard"><div class="whiteboard-lanes">${JOURNEY_STAGES.map((item) => `<div class="whiteboard-lane"><span>${esc(item.label)}</span><small>${esc(item.function)}</small></div>`).join('')}</div><div class="board-toolbar"><button class="primary-button" type="button" data-action="board-add">+ Add evidence</button><button class="secondary-button" type="button" data-action="board-arrange">Auto-arrange</button></div>
      ...
      ${!cards.length ? `<div class="empty-state" style="position:absolute;inset:80px 20px 20px"><div><strong>The shared board is empty</strong><span>Add evidence, a journey leak, a signal conflict, an opportunity, or a question to validate.</span><div style="margin-top:14px"><button class="primary-button" type="button" data-action="board-add">Add the first card</button></div></div></div>` : ''}
    </div><div class="callout" style="margin-top:14px"><strong>Evidence standard</strong><p>Label each card as confirmed evidence, a directional signal, or a hypothesis. Strategic confidence should never exceed the quality of the evidence.</p></div></section>`;
}
...
function renderWheel() {
  ...
  return `<section class="scene wheel-scene">${sceneHeader('LIVE ARENA · OPERATOR SELECTOR','Strategic reasoning under <em>shared pressure.</em>','The facilitator selects a category, the system selects a participant, and the room scores the answer for accuracy, clarity, and strategic defensibility.', isFacilitator()?`<button class="primary-button" type="button" data-action="wheel-spin" ${spinning?'disabled':''}>${spinning?'Selection in progress…':'Select operator'}</button>`:'')}
    <div class="wheel-layout"><section class="operator-wheel${spinning?' is-spinning':''}"><div class="wheel-radar"></div><div class="wheel-core"><small>${spinning?'SELECTION SEQUENCE':spin?'SELECTED OPERATOR':'ROOM READY'}</small><strong>${esc(spinning?'Scanning the room…':spin?.winnerName||`${people.length} participants`)}</strong><span>${spinning?'Matching participant and challenge category':spin?esc(challenge?.label||'Challenge'):'Choose a category and initiate selection'}</span></div>${people.map((person,index)=>`<div class="operator-node${!spinning&&spin?.winnerId===person.userId?' is-winner':''}" style="--i:${index};--count:${Math.max(people.length,1)};--operator:${attr(person.color||'#2864dc')}"><i>${esc(initials(person.name))}</i><span>${esc(person.name)}</span></div>`).join('')}</section>
      <aside class="panel wheel-console"><p class="eyebrow">CHALLENGE CATEGORY</p><div class="category-grid">${categories.map(([id,label])=>`<button type="button" class="chip-button${state.wheelCategory===id?' is-active':''}" data-action="wheel-category" data-category="${id}">${esc(label)}</button>`).join('')}</div><div class="wheel-challenge${spinning?' is-processing':''}">${spinning?`<span class="tag direction">Selection sequence active</span><h2>Analyzing the live participant field.</h2><p>The selected operator and challenge will be revealed room-wide.</p><div class="selection-sequence"><i></i><i></i><i></i><i></i><i></i></div>`:spin?`<span class="tag present">${esc(challenge?.label||'Challenge')}</span><h2>${esc(challenge?.prompt||'')}</h2><p>${esc(challenge?.guide||'')}</p>`:`<span class="tag direction">Awaiting selection</span><h2>The room is ready.</h2><p>Select a challenge category, then use the Operator Selector.</p>`}</div>${!spinning&&spin?`<form id="wheel-response-form" class="stack"><label><span>${canAnswer?'Operator response':'The selected operator is responding'}</span><textarea name="answer" rows="5" maxlength="700" required ${canAnswer?'':'disabled'} placeholder="Respond in clear, client-ready language."></textarea></label><button class="primary-button" type="submit" ${canAnswer?'':'disabled'}>Share response</button></form>`:''}</aside></div>
    <div class="panel-grid" style="margin-top:16px"><section class="panel span-7"><div class="flex-between"><div><p class="eyebrow">ROOM SCORING</p><h3>${latest?'Score the latest response':'No response submitted yet'}</h3></div>${latest?`<span class="tag">${esc(payloadOf(latest).winnerName)}</span>`:''}</div><div class="divider"></div>${latest?`<blockquote class="response-quote">“${esc(payloadOf(latest).answer)}”</blockquote><div class="wheel-score-grid">${['accuracy','clarity','strategy'].map((dim)=>{const stats=wheelRatingSummary(latest.id);return `<div><strong>${dim==='strategy'?'Strategic defensibility':dim[0].toUpperCase()+dim.slice(1)}</strong><div class="score-buttons">${[1,2,3,4,5].map((value)=>`<button type="button" data-action="wheel-rate" data-response="${latest.id}" data-dimension="${dim}" data-value="${value}" class="${myVote(`wheel:${latest.id}:${dim}`)?.value==value?'is-selected':''}">${value}</button>`).join('')}</div><small>${stats[dim].count?`${stats[dim].avg.toFixed(1)} average · ${stats[dim].count} ratings`:'Awaiting ratings'}</small></div>`}).join('')}</div>`:'<div class="empty-state"><div><strong>Run the first challenge</strong><span>The selected participant’s response will appear here for room scoring.</span></div></div>'}</section>
      <aside class="panel span-5"><p class="eyebrow">OPERATOR LEADERBOARD</p><h3>Average room score</h3><div class="leaderboard" style="margin-top:16px">${leaders.map((leader,index)=>`<div class="leader-row"><span class="rank">${String(index+1).padStart(2,'0')}</span><span class="avatar" style="background:${attr(leader.color||'#2864dc')}">${esc(initials(leader.name))}</span><strong>${esc(leader.name)}</strong><output>${leader.avg.toFixed(1)}/5</output></div>`).join('')||'<div class="empty-state"><div><strong>No scored responses yet</strong><span>Scores accumulate as the room completes challenges.</span></div></div>'}</div></aside></div>
  </section>`;
}
...
function renderAuction() {
  ...
  return `<section class="scene auction-scene">${sceneHeader('ACT IV · RESOURCE TRADEOFFS','You have 100 visibility credits. <em>Spend them deliberately.</em>','Allocate a constrained budget, save it room-wide, and compare how the team’s assumptions change between a visual-commerce brand and a high-stakes legal decision system.', `<div class="client-switch"><button type="button" data-action="auction-client" data-client="breezy" class="${state.auctionClient==='breezy'?'is-active':''}">Breezy Golf</button><button type="button" data-action="auction-client" data-client="kp" class="${state.auctionClient==='kp'?'is-active':''}">K&P Attorney</button></div>`)}
    ...
    <div class="panel-grid" style="margin-top:16px"><section class="panel span-7"><div class="flex-between"><div><p class="eyebrow">ROOM-WIDE MODEL</p><h2>${submissions.length} saved allocation${submissions.length===1?'':'s'}</h2></div><span class="tag present">${esc(client.name)}</span></div><div class="divider"></div><div class="aggregate-chart">${SIGNALS.map((signal)=>{const avg=aggregate[signal.id]/denom;return `<div class="aggregate-row"><label>${esc(signal.name)}</label><div class="result-track"><div class="result-fill" style="width:${avg/max*100}%;background:linear-gradient(90deg,#12b7ff,#315eff)"></div></div><output>${avg.toFixed(1)}</output></div>`}).join('')}</div></section>
      <aside class="panel span-5"><p class="eyebrow">PARTICIPANT ALLOCATIONS</p><div class="allocation-list">${submissions.map((item)=>{const p=payloadOf(item);const top=SIGNALS.map((signal)=>({name:signal.name,value:Number(p.allocations?.[signal.id]||0)})).sort((a,b)=>b.value-a.value).slice(0,3);return `<article><header><span class="avatar" style="background:${attr(p.ownerColor||'#2864dc')}">${esc(initials(p.ownerName||'P'))}</span><div><strong>${esc(p.ownerName||'Participant')}</strong><small>${top.map((x)=>`${x.name} ${x.value}`).join(' · ')}</small></div></header><output>${Object.values(p.allocations||{}).reduce((a,b)=>a+Number(b),0)}</output></article>`}).join('')||'<div class="empty-state"><div><strong>No allocations saved yet</strong><span>Every participant can submit a 100-credit model and compare it live.</span></div></div>'}</div><div class="callout"><strong>Discussion prompt</strong><p>Which allocations are stable across both clients, and which should change because risk, surface role, and evidence requirements differ?</p></div></aside></div>
  </section>`;
}
...
function renderDualVision() {
  ...
  return `<section class="scene decoder-scene">${sceneHeader('ACT IV · HUMAN + MACHINE SIGNAL DECODER','Translate perception into <em>observable evidence.</em>','Users experience relevance, risk, confidence, and effort. Systems interpret observable signals. Strategy must connect the two without pretending they are the same thing.')}
    ...
    <section class="panel chain-builder"><div class="flex-between"><div><p class="eyebrow">BUILD A DEFENSIBLE SIGNAL CHAIN</p><h3>Connect one human need to one business effect</h3></div><span class="tag direction">Saved room-wide</span></div><form id="signal-chain-form" class="chain-form"><label><span>Human perception</span><select name="human">${humanOptions}</select></label><label><span>Observable behavior</span><select name="behavior">${observable.map((x)=>`<option>${esc(x)}</option>`).join('')}</select></label><label><span>Machine-readable signal</span><select name="machine">${optionMarkup(SIGNALS,'id','name')}</select></label><label><span>Business outcome</span><select name="outcome">${outcomes.map((x)=>`<option>${esc(x)}</option>`).join('')}</select></label><label class="chain-rationale"><span>Why this chain is plausible</span><input name="rationale" maxlength="300" required placeholder="State the mechanism and evidence limitation."></label><button class="primary-button" type="submit">Save signal chain</button></form><div class="chain-wall">${connections.map((item)=>{const p=payloadOf(item);return `<article class="signal-chain-card"><div><span>${esc(p.humanName)}</span><b>→</b><span>${esc(p.behavior||'Observable behavior')}</span><b>→</b><span>${esc(p.machineName)}</span><b>→</b><span>${esc(p.outcome||'Business outcome')}</span></div><p>${esc(p.rationale||'')}</p><small>${esc(p.ownerName||'Team contribution')}</small></article>`}).join('')||'<div class="empty-state"><div><strong>No signal chains yet</strong><span>Create one chain, then challenge whether the evidence actually supports the claimed business effect.</span></div></div>'}</div></section>
...
function renderDebrief() {
  ...
  return `<section class="scene">${sceneHeader('FINAL ACT · LOCK IN THE MODEL','Convert the framework into a <em style="color:var(--lamark);font-style:normal">repeatable operating standard.</em>','Consolidate the decisions, document the evidence, and define how the framework will be applied on the next account.', `<a class="secondary-button" href="./assets/search-everywhere-operating-model.svg" download>Download operating model</a><button class="secondary-button" type="button" data-action="print-session">Print summary</button><button class="primary-button" type="button" data-action="export-session">Export workshop CSVs</button>`)}
